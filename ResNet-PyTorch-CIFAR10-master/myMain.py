...
def main(args):
    ...
    modelName = 'resnet34' # %d'%(args.n * 6 +2)
    ...
    # train model
    SCHEDULE_EPOCHS = [20, 10, 10]  # divide lr by 10 after each number of epochs
    total_epochs = sum(SCHEDULE_EPOCHS)
    #     SCHEDULE_EPOCHS = [100, 50, 50] # divide lr by 10 after each number of epochs
    history = np.zeros((total_epochs, 4))  # train_loss, valid_loss, train_acc, valid_acc
    learning_rate = 0.1
    start_epoch = 0
    for i, num_epochs in enumerate(SCHEDULE_EPOCHS):
        ...
        for epoch in range(num_epochs):
            ...
            train_loss, train_acc = train(loader_train, model, criterion, optimizer)

            print('On Validation Set ', end='')
            valid_loss, valid_acc = check_accuracy(model, loader_val, criterion)

            history[start_epoch + epoch, :] = np.array([train_loss, valid_loss, train_acc, valid_acc])
            checkpoint = {'model': model.state_dict(), 'optimizer': optimizer.state_dict(),
                          'epoch': start_epoch + epoch + 1}
            torch.save(checkpoint, os.path.join(trainedModelDir, dataset + '_model_%d.pth' % (start_epoch + epoch + 1)))

        learning_rate *= 0.1

    np.save(os.path.join(trainedModelDir, dataset + '_history.npy'), history)

    best_model_idx = history[:, 3].argmax()
    print("Best Performance On Valid Set at Epoch %d with Accuracy %.2f%%" % (best_model_idx + 1, 100 * history[best_model_idx, 3]))
    checkpoint = torch.load(os.path.join(trainedModelDir, dataset + '_model_%d.pth' % (best_model_idx + 1)))
    model.load_state_dict(checkpoint['model'])
    print('Final test accuracy:')
    check_accuracy(model, loader_test, criterion)
    # plots
    xidx = np.arange(1, total_epochs + 1)
    plt.plot(xidx, history[:, 0:2])
    plt.legend(['Tr Loss', 'Val Loss'])
    plt.xlabel('Epoch Number')
    plt.ylabel('Loss')
    plt.axis([0, 1.05 * total_epochs, 0, 1.1 * history[:, 0:2].max()])
    plt.savefig(dataset + '_' + modelName + '_loss_curve_Epochs_%d.png' % total_epochs)
    plt.show()

    plt.plot(xidx, history[:, 2:4])
    plt.legend(['Tr Accuracy', 'Val Accuracy'])
    plt.xlabel('Epoch Number')
    plt.ylabel('Accuracy')
    plt.axis([0, 1.05 * total_epochs, 0, 1])
    plt.savefig(dataset + '_' + modelName + '_accuracy_curve_Epochs_%d.png' % total_epochs)
    plt.show()


def check_accuracy(model, loader, loss_func):
    num_correct = 0
    num_samples = 0
    check_loss_sum = 0.
    model.eval()
    with torch.no_grad():
        for X, y in loader:
            ...
            scores = model(X_var)
            loss = loss_func(scores, y_var)
            check_loss_sum += loss.item() * X_var.size(0)
    ...
    avg_loss = check_loss_sum / num_samples
    return avg_loss, acc
...
def train(loader_train, model, criterion, optimizer):
    ...
    for t, (X, y) in enumerate(loader_train):
        ...
        loss.backward()
        optimizer.step()
        train_loss += loss.item() * X_var.size(0)
    ...
    print("On Training Set Got %d / %d correct (%.2f%%)" % (num_correct, num_samples, 100 * train_acc))
